...
addpath(genpath('D:\Dropbox\96 Paper DISTINCT\1 algorithm design'))
addpath(genpath('D:\Dropbox\96 Paper DISTINCT\2 algorithm verification'))

addpath(genpath('/proj2/adriaantaal/96 Paper DISTINCT/2 algorithm verification'))
...
% DProbe = create_Probe(NPixel,xpitch,ypitchD,0,0);
EProbe = create_Probe(NPixel,xpitch,ypitchE,259.4,1);
% EProbe = create_Probe(NPixel,xpitch,ypitchE,0,0);
...
figure; imagesc(D*ETE*D'); title('Detection, Emission field overlap covariance')


%% Look at the PSF

%point source to generate PSF around
XYZpsf = [(X(end)-X(1))/2+X(1) (Y(end)-Y(1))/2+Y(1) (Z(end)-Z(1))/2+Z(1)];
[ xpsf, xXYZind, ~ ] = convert_fluoloc(X,Y,Z,XYZpsf);

%We can model distortions in the imaging volume as salt and pepper noise too
Dmedium = Agenv7_reflections(X,Y,Z,repmat(Dgparams,NPixel/size(Dgparams,1),1),NPixel,DProbe);
Dmedium = Dmedium./max(max(Dmedium)); 

%high frequency matrix with reflections (salt&pepper noise), precharacterized 
Dhf = Agenv7_reflections(X,Y,Z,repmat(Dgparams,NPixel/size(Dgparams,1),1),NPixel,DProbe);
Dhf = Dhf./max(max(Dhf)); 

%same medium distortions apply to Dhf
Dhf = Dhf + (Dmedium - D);

%the signal from the distorted volume
y = Dmedium*xpsf;

%calculate PSF for low freq case
PSF = (pinv(D)*y);
PSF = reshape(PSF,sizes)';
xPSF = PSF(xXYZind(2),:)';

%calculate PSF for high freq case
y2 = Dhf*xpsf;
PSF2 = (pinv(Dhf)*y2);
PSF2 = reshape(PSF2,sizes)';
xPSF2 = PSF2(xXYZind(2),:)';

%true psf if we wouldve known distortions (impossible)
PSFtrue = (pinv(Dmedium)*y);
PSFtrue = reshape(PSFtrue,sizes)';
xPSFtrue = PSFtrue(xXYZind(2),:)';


figure; imagesc(X,Y,PSF); title('PSF from low freq matrix ');
figure; imagesc(X,Y,PSF2); title('PSF from high freq matrix');
figure; imagesc(X,Y,PSFtrue); title('True PSF');

figure;hold on;
plot(X,xPSFtrue); plot(X,xPSF); plot(X,xPSF2);
legend('True PSF','Low freq matrix','High freq matrix')
title('xPSF');
...
%data parameters
L=[2 4 6 8 10 12 14 16];          %number of sources    
SNR = 100;      %for implementation of noise
nIter = 5;   %number of repetitions in the monte carlo
...
%         axis([X(1) X(end) Y(1) Y(end) Z(1) Z(end)])
        ...
        y = D*diag(xtrue)*Ek';
        noise=imnoise(y./10,'poisson');
%         noise = 0;
        y = y+noise;
        ydotE = DdotE*xtrue+noise(:,1);
...
accmean = nanmean(accuracy);


figure
plot(L,mean(accmean,3))
xlim([0 20])
ylim([0 1])
xlabel("Number of Sources (L)")
ylabel("Accuracy")

% accstd = std(accuracy,0,4,'omitnan');
% 
% figure
...
%plot the results
f1 = figure; hold on; set(gcf,'Color',[1 1 1]);
...
savefig(f1,'output/myscatter');
print(f1,'output/myscatter','-dpng');

save('output/DISTINCT_sim.mat','accuracy','tcompute','L','SNR','nIter','addSize','lambdadiv','q','sigma_min','sigma_decrease_factor','max_contrast_ratio','D','Ek','X','Y','Z');
