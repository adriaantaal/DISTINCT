...
nIter = 200;   %number of repetitions in the monte carlo
...
for ll = 1:length(L) 
    ...
    %     figure; semilogy(diag(S),'x')
        ...
        %b. ICopt, no extra emitter rows, faster but uses less information
            ...
            tic; [xhatIC, f1,f2] = ICOpt(DdotEk,ydotEk,lambdadotEk,0,Inf,0,1,addSize);
        ...
        %e. competitor #4 -> our own algorithm using the 
        %gradient descent internal solver, for comparison
            tic; [xhatDSv3,~,~,~] = DISTINCTv3(y,D,Ek,lambda,H,gradconstdiag,1,maxIter,maxIterInt,addSize,1e3);
            tcompute(8,ll,ii) = toc;
            disp(['Found ' num2str(numel(find(xhatDSv3))) ' locations'])
            %choose L maximum -> for exact comparison wth ground truth
            [~,DSv3ind] = sort(xhatDSv3,'descend'); xhatDSv3(DSv3ind(L(ll)+1:end)) = 0;
        ...
        accuracy(8,ll,ii) = some_accuracy_function(xhatDSv3,xtrue);
...
save(['myoutput_' mmdd '/DISTINCT_sim_smallL.mat'],'accuracy','tcompute','L','SNR','nIter','addSize','lambdadiv','q','sigma_min','sigma_decrease_factor','max_contrast_ratio','X','Y','Z','numBatchFile');
