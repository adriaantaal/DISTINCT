...
P1pinv = pinv(P1);
projPTP = P1pinv*P1;        %detector projection matrix

Q1pinv = pinv(Q1);
projQTQ = Q1pinv*Q1;        %detector projection matrix
...
figure; imagesc(QTQ); title('Detector matrix inner product')
figure; imagesc(PTP); title('Emitter matrix inner product')
...
for ll = 1:length(L) 
    for ii = 1:nIter
        ...
        Xtrue = reshape(xtrue,N,N);
        %2D detector-emitter formulation
        y = P2*Xtrue*Q2';
        noise = 0; y = y+noise;
        gradconst = P2'*y*Q2;
        sl0gradconst = P1pinv*y*Q1pinv';
        
        %1D formulation
        y1d = D*xtrue+noise; %equivalent

        %original formulation
        yorig = (P1*Xtrue*Q1' + P2*Xtrue*Q2').*M_net;
...
%         gcdsort = sort(gradconst,'descend'); lambda = gcdsort(Lhat)/lambdadiv;
%         gcdotEksort = sort(ydotEk'*DdotEk,'descend'); lambdadotEk = gcdotEksort(Lhat)/lambdadiv;
        
    lambda = max(max(gradconst))/lambdadiv;
    lambda1d = max(y1d'*D)/lambdadiv;

    %a. DISTINCT: 
        tic; [xhatDS,f1,f2,f1f2] = DISTINCTv3(y,D,Ek,lambda,H,gradconst,0,maxIter,maxIterInt,addSize);
    ...
    %c. 2D SL0, adapted for 2D. Already improved speedup ~2-3x 
        ...
        tic; xhatSL0=SL0_2D_ATv3(y, sigma_min, sigma_decrease_factor, mu_0, q, Dpinv, Ekpinv, projD, projEk, sl0gradconst, max_contrast_ratio);
    ...
    %gradient descent internal solver, for comparison
        tic; [xhatDSGD,f1,f2,f1f2] = DISTINCTv2(y,D,Ek,lambda,H,gradconst,1,maxIter,maxIterInt,addSize);
    ...
    %g. DISTINCT using direct inversion: 
        tic; [xhatDINV,f1,f2,f1f2] = DISTINCTv2(y,D,Ek,lambda,H,gradconst,2,maxIter,maxIterInt,addSize);
      ...
      %h. DISTINCT using linsolve: 
        tic; [xhatDSQP,f1,f2,f1f2] = DISTINCTv2(y,D,Ek,lambda,H,gradconst,3,maxIter,maxIterInt,addSize);
    ...
    %for IBIS, the answer can be negative
        ...
        accuracy(5,ll,ii) = some_accuracy_function(abs(xhatDSGD),xtrue);
...
save(['myoutput_' mmdd '/DISTINCT_IBIS_Haar1024.mat'],'accuracy','tcompute','L','SNR','nIter','addSize','lambdadiv','q','sigma_min','sigma_decrease_factor','max_contrast_ratio','X','Y','Z','numBatchFile','K');
