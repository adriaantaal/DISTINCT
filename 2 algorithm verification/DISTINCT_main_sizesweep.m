...
nIter = 80;   %number of repetitions in the monte carlo
...
for ll = 1:length(res)
    ...
    for ii = 1:nIter
        ...
        [~,~,XYZtrue,~,~,xtrue] = create_randomRGlocs2D(X,Y,Z,L,pi/2,X(1),Y(1));
...
%             tic; [xhatNIHT] = NIHT2D_ATv4(y,D,Ek,L);
%             tcompute(4,ll,ii) = toc;    
%             %choose L largest entries -> for exact comparison wth ground truth
%             [~,NIHTind] = sort(xhatNIHT,'descend'); xhatNIHT(NIHTind(L+1:end)) = 0;   
        ...
        %gradient descent internal solver, for comparison
            ...
            [~,DSGDind] = sort(xhatDSGD,'descend'); xhatDSGD(DSGDind(L+1:end)) = 0;
        ...
        %direct inversion method
            ...
            %choose L maximum -> for exact comparison wth ground truth
            [~,DINVind] = sort(xhatDINV,'descend'); xhatDINV(DINVind(L+1:end)) = 0;
        ...
        %g. DISTINCT using linsolve: 
            ...
            %choose L largest entries -> for exact comparison wth ground truth
            [~,DSQPind] = sort(xhatDSQP,'descend'); xhatDSQP(DSQPind(L+1:end)) = 0;
	...
	save(['myoutput_' mmdd '/DISTINCT_sim_sizesweep.mat'],'accuracy','tcompute','L','SNR','nIter','addSize','lambdadiv','q','sigma_min','sigma_decrease_factor','max_contrast_ratio','X','Y','Z','numBatchFile','K');
end
