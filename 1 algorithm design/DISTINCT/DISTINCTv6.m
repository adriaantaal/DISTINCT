function [xhatFull,f1,f2,f1f2] = DISTINCTv6(y,D,E,lambda,H,gradconstdiag,U,s,V,intSolver,maxIter,maxIterInt,addSize)
...
if ~exist('lambda','var')
    lambda = max(gradconstdiag)/10; end
...
for ii = 1:maxIter
    ...
    if intSolver == 0
        ...
        % xhat = solvediagIntProdv3(U,s,V,D(:,in_crowd),E(:,in_crowd),H(in_crowd,in_crowd),lambda)
