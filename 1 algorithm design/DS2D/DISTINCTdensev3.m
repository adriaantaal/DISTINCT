function [xhatFull,f1,f2,f1f2] = DISTINCTdensev3(y,P,Q,lambda,H,gradconst,intSolver,maxIter,maxIterInt,addSize,max_contrast_ratio)
...
if ~exist('max_contrast_ratio','var')
    max_contrast_ratio = 1e3; end

%initialization
xhatFull = zeros(size(P,2));
xhat = [];
old_in_crowd = [];
in_crowd = [];
f1 = zeros(maxIter,1); 
...
%     us = wthresh(us,'s',lambda);
    ...
    xhatFull(in_crowd) = 1; %just as a test
    emptyrows = (sum(xhatFull,1)==0);
    emptycols = (sum(xhatFull,2)==0);
    
    %make Psmall by deleting all zero rows and cols
    Psmall = P; Psmall(:,emptycols) = [];
    Qsmall = Q; Qsmall(:,emptyrows) = []; 

    %Solve F-norm term exactly on the subset
    if intSolver == 0
    elseif intSolver == 1    
        x0 = zeros(length(in_crowd),1); x0(1:length(old_in_crowd)) = xhat;
        [xhat,~] = lsqr(H,gradconst,[],maxIterInt,[],[]);
    elseif intSolver == 2 %the direct solution through long inversion
%         xhat = H(in_crowd,in_crowd)\gradconst(in_crowd);
        xhat = H\gradconst;
    elseif intSolver == 3
        xhat = linsolve(H,gradconst,opts);
